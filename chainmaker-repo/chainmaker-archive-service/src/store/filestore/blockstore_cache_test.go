...
var (
	...
	wantBatch = archive_utils.NewUpdateBatch()
)
...
func TestStoreCacheMgr_AddBlock(t *testing.T) {
	...
	tests := []struct {
		...
		// TODO: Add test cases.
		{
			...
			args: args{
				blockHeight: 1,
				updateBatch: archive_utils.NewUpdateBatch(),
			},
		},
	}
	for _, tt := range tests {
...
func TestStoreCacheMgr_DelBlock(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mgr.RWMutex = sync.RWMutex{}
			mgr.AddBlock(0, archive_utils.NewUpdateBatch())
			mgr.DelBlock(0)
			mgr.AddBlock(1, archive_utils.NewUpdateBatch())
			mgr.DelBlock(2)
		})
	}
}
...
func TestStoreCacheMgr_Get(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if tt.name == "get1" {
				batch1 := archive_utils.NewUpdateBatch()
				batch1.Put([]byte(tt.args.key), []byte(tt.args.key))
				mgr.AddBlock(1, batch1)
				batch2 := archive_utils.NewUpdateBatch()
...
func TestStoreCacheMgr_KVRange(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mgr.RWMutex = sync.RWMutex{}
			batch := archive_utils.NewUpdateBatch()
