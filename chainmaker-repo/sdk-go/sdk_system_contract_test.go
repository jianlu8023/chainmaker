...
func TestGetTxByTxIdTruncate(t *testing.T) {
	var txID = "172e31fea3c16160cac0e5cfbe2508dd6aa2bce28e1749588f0680a22168fc9e"
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cli.GetTxByTxIdTruncate(txID, true, 10000, "")

		})
	}
}

func TestGetBlockByHeightTruncate(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			blk, err := cc.GetBlockByHeightTruncate(height, true, 10000, "")
			require.Nil(t, err)
			bz, err := blk.Marshal()
			require.Nil(t, err)
			require.NotNil(t, bz)
