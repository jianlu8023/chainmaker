...
type ChainClientConfig struct {
	...
	archiveCenterHttpConfig *ArchiveCenterHttpConfig
}
...
func WithArchiveCenterHttpConfig(conf *ArchiveCenterHttpConfig) ChainClientOption {
	return func(config *ChainClientConfig) {
		config.archiveCenterHttpConfig = conf
	}
}
...
func setArchiveCenterConfig(config *ChainClientConfig) {
	if config.ConfigModel.ChainClientConfig.ArchiveCenterConfig != nil &&
		...
		config.archiveCenterHttpConfig = &ArchiveCenterHttpConfig{}
		config.archiveCenterHttpConfig.ChainGenesisHash = strings.TrimSpace(
			config.ConfigModel.ChainClientConfig.ArchiveCenterConfig.ChainGenesisHash)
		config.archiveCenterHttpConfig.ArchiveCenterHttpUrl = strings.TrimSpace(
		...
		if config.ConfigModel.ChainClientConfig.ArchiveCenterConfig.ReqeustSecondLimit > 0 {
			config.archiveCenterHttpConfig.ReqeustSecondLimit =
				config.ConfigModel.ChainClientConfig.ArchiveCenterConfig.ReqeustSecondLimit
		} else {
			config.archiveCenterHttpConfig.ReqeustSecondLimit = httpRequestDuration
		}
	}
}
